...
// Jadi tidak kena CORS!

export default async function handler(req, res) {
  // CORS headers — izinkan frontend kamu akses
  ...
  // ── Mode proxy: download file lalu stream ke client (bypass CORS di browser)
  if (proxy === '1' && url) {
    try {
  ...
  try {
    const r = await fetchTimeout(
      `https://www.tikwm.com/api/?url=${encodeURIComponent(url)}&hd=1`, 10000
    );
  ...
  try {
    const r = await fetchTimeout(
      `https://api.tiklydown.eu.org/api/download/v3?url=${encodeURIComponent(url)}`, 10000
    );
    const json = await r.json();
    if (json.data) {
      const d = json.data;
  ...
  try {
    const r = await fetchTimeout(
      `https://api.tiklydown.eu.org/api/download?url=${encodeURIComponent(url)}`, 10000
    );
    const json = await r.json();
    if (json.data) {
      const d = json.data;
